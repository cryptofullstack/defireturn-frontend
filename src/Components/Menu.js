...
import { SELECT_MENU, PORTFOLIO_DATA, FOOTNOTES_DATA, SET_LOADING } from "../actionTypes";
...
const Menu = ({ openMenu, updateMenu, allowedMenu }) => {
  ...
  useEffect(() => {
    if (typeof walletAddress != "undefined" && walletAddress.length > 0) {
      ...
      interval = setInterval(async () => {
        try {
          let response = await axios.get(
            `${SERVE_URL}/status/${walletAddress}`
          );
      ...
      axios
        .post(
          `${SERVE_URL}/wallet/${walletAddress}` //TODO: Multi chain
        )
        .then(() => { })
        .catch((e) => {
          // error try pls
        ...
        <nav className="navbar bg-light left-navbar">
          <ul className="navbar-nav">
            ...
            <Link className="nav-link" to="/help">
              ...
              >
                <img className="menu-icon" src="../assets/images/help.jpg" />
