...
const Portfolio = ({data, updateMenu, openMenu, allowedMenu, allowMenu}) => {
    ...
    function make_child_tree_structure(data, item, index, index1, num) {
        ...
        return <div className="row border-1">
            ...
            <div className="col-3 sub-table-body sub-table-body-text" title={data.token_url}>
                <a href={data.token_url} target="_blank" className="sub-table-body-text"> <CryptoIcon className="sub-table-image" type="color" name={data.token_img} />
                    {/* <img className="sub-table-image" src={data.token_img} /> */}
                    {data.token_name}</a>
            </div>
            <div className="col-2 sub-table-body sub-table-body-num">{(data.units > 0 ? '+' : "") + data.units_str}</div>
                    ...
                    <tbody>
                        {portfolio_table_body_data.map((item, index) => {
                            return <><tr key={index} className="accordion-toggle table-tr"
                                id="accordion1" >
                                {item.history ? item.history.length > 0 ? <td className="table-body expand-button td_symbol collapsed" data-toggle="collapse" href={`#${item.id + item.chain}`} ></td> :<td></td> : <td></td>}
                                <td className="table-body vertical-align-middle second-td"><img className="table-td-s-image" src={item.type_img} />{item.type}</td>
                                <td className="table-body table-body-protocol vertical-align-middle" title={item.protocol_url}>
                                    <a href={item.protocol_url} className="pt3" target="_blank">
